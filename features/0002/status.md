...
- **Checklist do usuário (nesta ordem)**:
  1. ✅ **Produção feita (2026-09-24)**: chave gerada pelo usuário (validada: Base64, 32 bytes) e aplicada pelo Claude com Terraform 1.16.4 em duas etapas com `-target` (1: secret + versão 1 + IAM `secretAccessor` p/ `cime-run-runtime`; 2: `cime-pullrequest` com a env `UserIntegrations__EncryptionKey` → secret `latest`). Revisão `cime-pullrequest-00006-5r9` Ready com 100% do tráfego (mesma imagem de antes). O serviço `auth` **não** foi tocado — o `plan` completo mostra só um drift cosmético (`scaling` de serviço com zeros) nele e no pullrequest, inofensivo. **Falta: a mesma chave no ambiente local** (user-secrets ou env var) — ver abaixo. Gerar a chave uma vez: `openssl rand -base64 32`. Local: variável de ambiente `UserIntegrations__EncryptionKey` (não no appsettings versionado). Produção: colocar em `secrets.auto.tfvars` (`"user-integrations-encryption-key" = "<chave>"`) **antes** do `terraform apply` — sem o valor, o apply falha (o `main.tf` já referencia o secret). **Nunca trocar depois de em uso.**
